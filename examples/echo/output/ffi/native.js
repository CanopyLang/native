...
function _Native_diffApplyFacts(nNode, xFacts, yFacts, eventNode) {
    ...
    for (var k2 in xPlain) { if (!(k2 in yPlain)) { delta[k2] = undefined; changed = true; } }
...
// Map keys → existing nNodes, update matches, recycle orphans, then reorder by the
// minimal set of moves. Simplified from the source's LIS pass: we move every node
// not already in place, which is correct (just not move-minimal). Optimize later
...
function _Native_updateKeyedKids(nNode, xKids, yKids, eventNode) {
    ...
    // reorder: ensure parent's children match newN order. Detach all, reinsert in
    // order — correct and simple; move-minimization is a later optimization.
    for (var d = 0; d < newN.length; d++) {
        _Native_insertChild(parent, newN[d].__handle, d);
    }
    nNode.__kids = newN;
}
